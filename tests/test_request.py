...
class TestYAMLReadOnly:
    """read_only=True swaps in ruamel's safe loader for files Kometa never save()s back out - see KOMETA-PERFORMANCE-PROFILING-PLAN.md Phase 5's post-checklist YAML-loader finding, 2026-07-11."""
...
def make_image_requests():
    """Bare Requests instance with just the attributes get_image() touches, plus its get()/head() cast to MagicMock for mock-attribute access (perf project, 2026-07-10 - same cast pattern test_plex.py uses for this exact pyright gotcha)."""
    req = make_requests()
...
class TestGetImageMemoization:
    """get_image() used to hit the network on every call, even for a URL already fetched this run - see KOMETA-PERFORMANCE-PROFILING-PLAN.md Phase 5 checklist item 5."""
...
class TestGetImageValidateOnly:
    """validate_only=True (builder.py's url_poster/url_background/url_logo/url_square_art checks) uses HEAD instead of GET - same status/Content-Type validation, without downloading the body (perf project, 2026-07-11)."""
...
class TestGetYamlReadOnly:
    """Requests.get_yaml() never sets a path (save() was already a no-op), so it always requests the fast read_only loader - see KOMETA-PERFORMANCE-PROFILING-PLAN.md Phase 5's post-checklist YAML-loader finding, 2026-07-11."""
