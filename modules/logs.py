...
class BufferedRotatingFileHandler(RotatingFileHandler):
    """Flushes every FLUSH_EVERY records or immediately for WARNING+ instead of after every single line - the default per-line flush() was a measured ~15s of self time inside compile_overlays with several handlers open at once. force_flush() bypasses the buffering for callers that need the file complete right now (handler removal, close)."""
